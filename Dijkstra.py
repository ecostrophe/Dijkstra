...
class Dijkstra:
	...
	def get_allPaths(self):
		for visit in self.visited_list:
			print(visit)
...
def show_point(win,vertex,color,weight=weight):
	...
	fontpos = pygame.font.Font('freesansbold.ttf',10)
	idvx= fontpos.render(str(vertex.id_vertex),True,color)
	txtpos= fontpos.render(str(v[0])+":"+str(v[1]),True,color)
	prepos= fontpos.render("B:"+str(vertex.previous_vertex[0])+":"+str(vertex.previous_vertex[1]),True,red)
...
while run:
	...
	#for stx in d.scaned_vertex:
		#pygame.draw.lines(win, red,False, [(stx[0].xpos,stx[0].ypos),(stx[1].xpos,stx[1].ypos)], 1)
	for p in d.all_vertex:
		if p.zone1:
	...
	show_point(win,d.startvx,red,weight)
	show_point(win,d.targetvx,green,weight)
	d.find_path(win)
	time.sleep(1)
	pygame.display.update()
